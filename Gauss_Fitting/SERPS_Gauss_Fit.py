...
if run_program == True:
	...
	# Get Image List
	imagelist=np.concatenate([['SERPS_2.lower.mask.deeper2.briggs.pbcorr.fits']*Source_Num,['SERPS_2.upper.mask.deeper2.briggs.pbcorr.fits']*Source_Num])
	...
	# Get Region List
	regions=[]
	with open('SS_Targets_ICRS.txt') as fp:
		for line,i in zip(fp,range(22)):
			if i == 0:
				continue
	...
	outputfilefits = open('Fit_Results.txt','w')
	...
	for i in range(len(sources)):
		...
		print '-'*25
		if i == 8 or i == 9 or i == 13 or i == 15 or i == 17 or i == 34:
...
if write_files == True:
	...
	casa_file = open('CASA_gaussians_lower.txt','w')
	casa_file.write('#CRTFv0 CASA Region Text Format version 0\n')
	ds9_file = open('ds9_gaussians_lower.reg','w')
	...
	for i in np.arange(0,Source_Num):
		...
		ds9_file.write('ellipse('+str(np.around(ra[i],5))+','+str(np.around(dec[i],5))+','+str(np.around(bmaj[i],5))+'",'+str(np.around(bmin[i],5))+'",'+str(np.around(pos_angle[i],5))+') # text={'+str(sourcename[i][13:])+'}\n')
	
	casa_file.close()
	ds9_file.close()

	# Write out files for upper
	casa_file = open('CASA_gaussians_upper.txt','w')
	casa_file.write('#CRTFv0 CASA Region Text Format version 0\n')
	ds9_file = open('ds9_gaussians_upper.reg','w')
	ds9_file.write('# Region file format: DS9 version 4.1\n')
	ds9_file.write('global color=green dashlist=8 3 width=1 font="helvetica 10 normal roman" select=1 highlite=1 dash=0 fixed=0 edit=1 move=1 delete=1 include=1 source=1\n')
	ds9_file.write('fk5\n')
